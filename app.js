...
;(() => {
  ...
  function bindUI(){
    if (el.scanInput){
      ...
      el.scanInput.addEventListener("input", () => ensureAudio());
    }
  ...
  function renderLast(){
    ...
    const recent = scans.slice(0, 10).map(s => `<span class="${s.ok?'ok':'err'}">${s.ok?'✓':'✗'} ${escapeHtml(s.code)}</span>`).join(" · ");
